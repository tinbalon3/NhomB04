var arrayOfAroma = [
    {
        id: 2,
        name: 'FRENCH PRESS',
        price: '$23.99',
        priceCurrent:23,
        image: '../img/productList-img-21.png',
        link: '../html/Detail_FrenchPress.html',
        type:2
    },
    {
        id: 4,
        name: 'DOLCE GUSTO',
        price: '$12.00',
        priceCurrent:12,
        image: '../img/backgroundCopy_1.png',
        link: '../html/Detail_DolceGusto.html',
        type:4
    },
    {
        id: 8,
        name: 'Guatemala Coffee',
        price: '$25.00',
        priceCurrent:25,
        image: '../img/product-img-8.png',
        link:'../html/Detail_Guatemala.html',
        type:3
    },
    {
        id: 10,
        name: 'Indonesia Coffee',
        price: '$30.00',
        priceCurrent: 30,
        image: '../img/product-img-10.png',
        link: '../html/Detail_IndonesiaCoffee.html',
        type: 3
    }
]

var arrayOfBeans = [
    {
        id: 1,
        name: 'FRENCH PRESS',
        price: '$23.99',
        priceCurrent:23,
        image: '../img/productList-img-21.png',
        link: '../html/Detail_FrenchPress.html',
        type:2
    },
    {
        id: 2,
        name: 'Guatemala Coffee',
        price: '$25.00',
        priceCurrent:25,
        image: '../img/product-img-8.png',
        link:'../html/Detail_Guatemala.html',
        type:3
    },
    {
        id: 3,
        name: 'Brazil Coffee',
        price: '$35.00',
        priceCurrent: 35,
        image: '../img/product-img-11.png',
        link: '../html/Detail_BrazilCoffee.html',
        type: 3
    }
]

var arrayOfBlack = [
    {
        id: 1,
        name: 'ETHIOPIA COFFEE',
        price: '$15.99',
        priceCurrent:15,
        image: '../img/productList-img-1.png',
        link:'../html/Detail_ProductList.html',
        type:1
    },
    {
        id: 3,
        name: 'KENYA COFFEE',
        price: '$18.00',
        priceCurrent:18,
        image: '../img/productList-img-3.png',
        link: '../html/Detail_KenyaCoffee.html',
        type:3
    },
    {
        id: 5,
        name: 'Columbia Coffee',
        price: '$21.00',
        priceCurrent:21,
        image: '../img/productList-img-5.png',
        link:'../html/Detai_ColumbiaCoffee.html',
        type:4
    },
    {
        id: 7,
        name: 'Espresso Machine',
        price: '$600.00',
        priceCurrent:600,
        image: '../img/ProductList-img-7.png',
        link:'../html/Detail_EspressoMachine.html',
        type:0
    },
    {
        id: 9,
        name: 'Filter handle',
        price: '$80.00',
        priceCurrent:80,
        image: '../img/ProductList-img-9.png',
        link:'../html/Detail_FilterHandle.html',
        type:3
    },
    {
        id: 11,
        name: 'Brazil Coffee',
        price: '$35.00',
        priceCurrent: 35,
        image: '../img/product-img-11.png',
        link: '../Detail_BrazilCoffee.html',
        type: 3
    }
]
var arrayOfCasual = [
    {
        id: 1,
        name: 'ETHIOPIA COFFEE',
        price: '$15.99',
        priceCurrent:15,
        image: '../img/productList-img-1.png',
        link:'../html/Detail_ProductList.html',
        type:1
    },
    {
        id: 3,
        name: 'KENYA COFFEE',
        price: '$18.00',
        priceCurrent:18,
        image: '../img/productList-img-3.png',
        link: '../html/Detail_KenyaCoffee.html',
        type:3
    },
    {
        id: 5,
        name: 'Columbia Coffee',
        price: '$21.00',
        priceCurrent:21,
        image: '../img/productList-img-5.png',
        link:'../html/Detai_ColumbiaCoffee.html',
        type:4
    },
    {
        id: 6,
        name: 'Coffee Kettle',
        price: '$90.00',
        priceCurrent:90,
        image: '../img/ProductList-img-6.png',
        link:'../html/Detail_CoffeeKettle.html',
        type:1
    },
    {
        id: 11,
        name: 'Brazil Coffee',
        price: '$35.00',
        priceCurrent: 35,
        image: '../img/product-img-11.png',
        link: '../html/Detail_BrazilCoffee.html',
        type: 3
    },
    {
        id: 12,
        name: 'Mental Cup',
        price: '$23.00',
        priceCurrent: 23,
        image: '../img/ProductList-img-MetalCup.png',
        link: '../html/Detai_MeatlCup.html',
        type: 3
    },
    {
        id: 13,
        name: 'Dispensing Tray',
        price: '$30.00',
        priceCurrent: 30,
        image: '../img/ProductList-img-DespensingTray.png',
        link: '../html/Detail_DispensingTray.html',
        type: 3
    },
    {
        id: 14,
        name: 'Measuring Cup',
        price: '$16.00',
        priceCurrent: 16,
        image: '../img/ProductList-img-MeasuringCup.png',
        link: '../html/Detail_MeasuringCup.html',
        type: 3
    },
    {
        id: 15,
        name: 'Ceramic Cup',
        price: '$37.00',
        priceCurrent: 37,
        image: '../img/Productlist-img-CeramicCup.png',
        link: '../html/Detail_CeramicCup.html',
        type: 3
    }
]

var arrayOfClassic = [
    {
        id: 1,
        name: 'ETHIOPIA COFFEE',
        price: '$15.99',
        priceCurrent:15,
        image: '../img/productList-img-1.png',
        link:'../html/Detail_ProductList.html',
        type:1
    },
    {
        id: 3,
        name: 'KENYA COFFEE',
        price: '$18.00',
        priceCurrent:18,
        image: '../img/productList-img-3.png',
        link: '../html/Detail_KenyaCoffee.html',
        type:3
    },
    {
        id: 5,
        name: 'Columbia Coffee',
        price: '$21.00',
        priceCurrent:21,
        image: '../img/productList-img-5.png',
        link:'../html/Detai_ColumbiaCoffee.html',
        type:4
    },
    {
        id: 7,
        name: 'Espresso Machine',
        price: '$600.00',
        priceCurrent:600,
        image: '../img/ProductList-img-7.png',
        link:'../html/Detail_EspressoMachine.html',
        type:0
    },
    {
        id: 9,
        name: 'Filter handle',
        price: '$80.00',
        priceCurrent:80,
        image: '../img/ProductList-img-9.png',
        link:'../html/Detail_FilterHandle.html',
        type:3
    }
]

var arrayOfCream = [
    {
        id: 6,
        name: 'Coffee Kettle',
        price: '$90.00',
        priceCurrent:90,
        image: '../img/ProductList-img-6.png',
        link:'../html/Detail_CoffeeKettle.html',
        type:1
    },
    {
        id: 7,
        name: 'Espresso Machine',
        price: '$600.00',
        priceCurrent:600,
        image: '../img/ProductList-img-7.png',
        link:'../html/Detail_EspressoMachine.html',
        type:0
    },
    {
        id: 9,
        name: 'Filter handle',
        price: '$80.00',
        priceCurrent:80,
        image: '../img/ProductList-img-9.png',
        link:'../html/Detail_FilterHandle.html',
        type:3
    },
    {
        id: 12,
        name: 'Mental Cup',
        price: '$23.00',
        priceCurrent: 23,
        image: '../img/ProductList-img-MetalCup.png',
        link: '../html/Detai_MeatlCup.html',
        type: 3
    },
    {
        id: 13,
        name: 'Dispensing Tray',
        price: '$30.00',
        priceCurrent: 30,
        image: '../img/ProductList-img-DespensingTray.png',
        link: '../html/Detail_DispensingTray.html',
        type: 3
    },
    {
        id: 15,
        name: 'Ceramic Cup',
        price: '$37.00',
        priceCurrent: 37,
        image: '../img/Productlist-img-CeramicCup.png',
        link: '../html/Detail_CeramicCup.html',
        type: 3
    },
    {
        id: 16,
        name: 'Coffee Capsule',
        price: '$12.00',
        priceCurrent: 12,
        image: '../img/productList-img-CoffeeCapsule.png',
        link: '../html/Detail_CoffeeCapsule.html',
        type: 3
    }
]

var arrayOfElegant = [
    {
        id: 6,
        name: 'Coffee Kettle',
        price: '$90.00',
        priceCurrent:90,
        image: '../img/ProductList-img-6.png',
        link:'../html/Detail_CoffeeKettle.html',
        type:1
    },
    {
        id: 7,
        name: 'Espresso Machine',
        price: '$600.00',
        priceCurrent:600,
        image: '../img/ProductList-img-7.png',
        link:'../html/Detail_EspressoMachine.html',
        type:0
    },
    {
        id: 9,
        name: 'Filter handle',
        price: '$80.00',
        priceCurrent:80,
        image: '../img/ProductList-img-9.png',
        link:'../html/Detail_FilterHandle.html',
        type:3
    },
    {
        id: 12,
        name: 'Mental Cup',
        price: '$23.00',
        priceCurrent: 23,
        image: '../img/ProductList-img-MetalCup.png',
        link: '../html/Detai_MeatlCup.html',
        type: 3
    },
    {
        id: 13,
        name: 'Dispensing Tray',
        price: '$30.00',
        priceCurrent: 30,
        image: '../img/ProductList-img-DespensingTray.png',
        link: '../html/Detail_DispensingTray.html',
        type: 3
    },
    {
        id: 15,
        name: 'Ceramic Cup',
        price: '$37.00',
        priceCurrent: 37,
        image: '../img/Productlist-img-CeramicCup.png',
        link: '../html/Detail_CeramicCup.html',
        type: 3
    },
    {
        id: 16,
        name: 'Coffee Capsule',
        price: '$12.00',
        priceCurrent: 12,
        image: '../img/productList-img-CoffeeCapsule.png',
        link: '../html/Detail_CoffeeCapsule.html',
        type: 3
    }
]

var arrayOfFresh = [
    {
        id: 4,
        name: 'DOLCE GUSTO',
        price: '$12.00',
        priceCurrent:12,
        image: '../img/backgroundCopy_1.png',
        link: '../html/Detail_DolceGusto.html',
        type:4
    },
    {
        id: 10,
        name: 'Indonesia Coffee',
        price: '$30.00',
        priceCurrent: 30,
        image: '../img/product-img-10.png',
        link: '../html/Detail_IndonesiaCoffee.html',
        type: 3
    }
]

function RenderFirstAroma()
   {
    var firstpage =arrayOfAroma.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstBeans()
   {
    var firstpage =arrayOfBeans.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstBlack()
   {
    var firstpage =arrayOfBlack.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstCasual()
   {
    var firstpage =arrayOfCasual.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstClassic()
   {
    var firstpage =arrayOfClassic.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstCream()
   {
    var firstpage =arrayOfCream.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstElegant()
   {
    var firstpage =arrayOfElegant.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
}

function RenderFirstFresh()
   {
    var firstpage =arrayOfFresh.map(function(product1)
    {
        return  `
                    <div class="body__product col-lg-4" data-aos="fade-up">
                    <div class="body__product-img-content">
                        <img src="${product1.image}" alt="Product image" class="body__product-img">
                        <div class="body__product-img--modal">
                            <div class="body__product-button--modal js-BtnAdd" onclick="rename(this,{
                                name: '${product1.name}',
                                price:${product1.price},
                                img:'${product1.image}',
                                qty:1,
                            })">
                                <p class="body__product-img--modal-link body__product-button-text--modal ">Add to cart</p>
                            </div>
                        </div>
                    </div>
    
                    <div class="body__product-text"> 
                        <h2 class="body__product-name">${product1.name}</h2>
                        <h3 class="body__product-price">${product1.price}</h3>
                    </div>
                </div>
               
                    `
    })
    productList.innerHTML = firstpage.join('');
    document.querySelector('.body__page-number-list').style.display ='none';
   }

